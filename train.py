...
if __name__ == '__main__':
    ...
    flags.DEFINE_integer("batch_size", 96, "Number of examples to process per batch \
                            for training")
    flags.DEFINE_integer("num_epochs", 100, "How many passes to make over the dataset \
    ...
    flags.DEFINE_integer("max_steps", None, "The maximum number of iterations of the \
                            training loop")
    flags.DEFINE_integer("learning_rate_decay_examples", 377790, "Multiply current learning \
    ...
    flags.DEFINE_float("learning_rate_decay", 1, "Learning rate decay factor to be \
...
def build_graph(reader,
    ...
    predictions = ( logits_aux_0 + logits_aux_1 )
        ...
        with sv.managed_session(config=self.config) as sess:
            try:
                ...
                while (not sv.should_stop()) and (not self.max_steps_reached):
                    ...
                    six_inputs = [np.reshape(x, [-1, 224, 224, 3]) for x in six_inputs]
		    ...
		    feats_for_aux = []
                    ...
                    aux_fcs = [np.concatenate([feats_for_aux[i], feats_for_aux[i+2],
